...
- Create a docker image repository `todo_app` on AWS ECR from Management Console.
```text
Visibility settings     : Private
Repository name         :  <your account ID>.dkr.ecr.us-east-1.amazonaws.com/todo_app
...
- Go to the `todo_app-node-project` repository page and click on `Settings`.
...
- Enter `todo_app-pipeline` then select `Pipeline` and click `OK`.
...
```text
https://github.com/xxxxxxxx/todo_app-node-project.git
```
...
```text
https://github.com/xxxxxxxxxxx/todo_app-node-project.git
```
...
- Go to the Jenkins instance (todo_app-node-project/ directory) to create `Jenkinsfile`

```bash
cd todo_app-node-project/
ls
vi Jenkinsfile

Press "i" to edit 
```
- Create a `Jenkinsfile` within the `todo_app-node-project` repo with following pipeline script. 
```groovy
pipeline {
    agent any
    stages {
...
- Go under todo_app-node-project/ folder and then create a Docker file via `vi` editor.
...
- Paste  the following content within a Dockerfile which will be located in `todo_app-node-project` repository.
...
```bash
docker run --name todo -dp 80:3000 <aws_account_id>.dkr.ecr.us-east-1.amazonaws.com/todo_app:latest
```
...
- Change a `Jenkinsfile` within the `todo_app-node-project` repo and add Deploy stage like below.
...
```bash
aws ecs create-cluster --cluster-name todo_app
```

- Create a task definition file and name it as todo_app.json. Don't forget the `<aws-account-id>` parts.
...
```json
{
	...
	"containerDefinitions": [{
		"name": "todo_app",
		"image": "<aws-account-id>.dkr.ecr.us-east-1.amazonaws.com/todo_app:latest",
		"portMappings": [{
			"containerPort": 3000,
			"protocol": "tcp"
		}],
...
```bash
aws ecs register-task-definition --cli-input-json file://todo_app.json
```
...
```bash
aws ecs create-service --cluster todo_app --service-name todo_app-service --task-definition todo_app --desired-count 1 --launch-type "FARGATE" --network-configuration "awsvpcConfiguration={subnets=[subnet-09e68d6993d05962d],securityGroups=[sg-0e7b20da367efc8e7],assignPublicIp=ENABLED}" # change the subnets and security group and make sure that the 3000 port is open. 
```

> Note: securityGroups=[sg-0e7b20da367efc8e7] is default sg. If we don't specify any sg, aws assign default sg to the cluster. Port #3000 should be allowed on the sg.
...
pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sh 'aws ecs update-service --cluster todo_app --desired-count 1 --service todo_app-service --task-definition todo_app --force-new-deployment'
            }
        }
    }
    post {
...
- Delete the ECR repository `todo_app` from AWS CLI.

```bash
aws ecr delete-repository \
      --repository-name todo_app \
      --force \
      --region us-east-1
```
...
```bash
aws ecs delete-service --cluster todo_app --service todo_app-service --force
aws ecs delete-cluster --cluster todo_app
```
